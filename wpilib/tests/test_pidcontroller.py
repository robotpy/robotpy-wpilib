...
def test_pidcontroller_init_args4(wpilib, SimTimerTask):
    source = lambda: 77.0
    output = MagicMock()
    pid = wpilib.PIDController(period=5.0, Ki = 2.0, Kp = 1.0, Kd = 3.0, Kf = 4.0, source=source, output=output)
...
def test_pidcontroller_init_args5(wpilib, SimTimerTask):
    ...
    assert exinfo.value.args[0] == "__init__() missing 1 required positional argument: 'Kp'"
...
def test_pidcontroller_init_args6(wpilib, SimTimerTask):
    ...
    assert exinfo.value.args[0] == "__init__() missing 1 required positional argument: 'Ki'"
...
def test_pidcontroller_init_args7(wpilib, SimTimerTask):
    ...
    assert exinfo.value.args[0] == "__init__() missing 1 required positional argument: 'Kd'"
...
def test_pidcontroller_init_args8(wpilib, SimTimerTask):
    ...
    assert exinfo.value.args[0] == "__init__() missing 3 required positional arguments: 'Kp', 'Ki', and 'Kd'"
...
def test_pidcontroller_init_args9(wpilib, SimTimerTask, sim_print):
        ...
        assert exinfo.value.args[0] == "Attribute error, attributes given did not match any argument templates. See messages above for more info"
        ...
        sim_print.assert_has_calls([
            ...
            call(str_like("  output: value [^,]+, type <class 'unittest.mock.MagicMock'>")),
            call('**************************************************'),
...
def test_pidcontroller_init_args10(wpilib, SimTimerTask):
    source = lambda: 77.0
    output = MagicMock()
    pid = wpilib.PIDController(Ki = "2.0", Kp = 1.0, Kd = 3.0, Kf = 4.0, source=source, output=output)
...
def test_pidcontroller_init_args11(wpilib, SimTimerTask, sim_print):
    ...
    sim_print.assert_has_calls([
        ...
        call(str_like("  source: value <function test_pidcontroller_init_args11.<locals>.<lambda> at [^>]+>, type <class 'function'>")),
        call(str_like("  output: value .+, type <class 'unittest.mock.MagicMock'>")),
        call('**************************************************'),
        call('Checking template 0: Kf, source, output, period'),
        call("- Error at arg 0: Kf != <class 'float'> or <class 'int'>"),
...
def test_pidcontroller_init_source1(wpilib, SimTimerTask):
    ...
    assert pid.pidInput.getPIDSourceType() == wpilib.interfaces.pidsource.PIDSource.PIDSourceType.kDisplacement
...
def test_pidcontroller_init_source2(wpilib, SimTimerTask):
    ...
    assert pid.pidInput.getPIDSourceType() == wpilib.interfaces.pidsource.PIDSource.PIDSourceType.kRate
...
def test_pidcontroller_calculate_rate2(pid):
    ...
    pid.pidOutput.assert_called_with(.875)
...
@pytest.mark.parametrize('continuous, input, setpoint, expected_error, expected_output', [
...
@pytest.mark.parametrize('i, source1, source2, output1, output2, output3', [
    (1.0, 49.5, 49.9, 0.5, 1.0, 1.0),
    (0.5, 49.5, 49.9, 0.25, 0.5, 0.55),
    (1.0, 49.5, 50.6, 0.5, 1.0, 0.4),
    ])
def test_pidcontroller_calculate_displacement2(pid, i, source1, source2, output1, output2, output3):
...
@pytest.mark.parametrize('d, source1, source2, output1, output2, output3', [
    (1.0, 49.5, 49.9, 0.5, 0.0, -0.4),
    (0.5, 49.5, 49.9, 0.25, 0.0, -0.2),
    (1.0, 49.5, 50.6, 0.5, 0.0, -1.0),
    ])
def test_pidcontroller_calculate_displacement3(pid, d, source1, source2, output1, output2, output3):
...
@pytest.mark.parametrize('f, source1, source2, output1, output2, output3', [
...
@pytest.mark.parametrize('f, setpoint, expected', [
    (1.0, 50.0, 50.0),
    (0.5, 50.0, 25.0),
    ])
...
def test_pidcontroller_calculateFeedForward_displacement(sim_hooks, wpilib, SimTimerTask):
    sim_hooks.time = 1.0
