...
def test_ultrasonic_ping(ultrasonic, wpilib, hal_data):
    ultrasonic.ping()
    assert hal_data['dio'][1]['pulse_length'] == pytest.approx(wpilib.Ultrasonic.kPingTime, 0.01)

    assert ultrasonic.isRangeValid() == False
    hal_data['counter'][0 # wat? should be 2?
        ]['count'] = 5
    assert ultrasonic.isRangeValid() == True
...
def test_ultrasonic_pidsourcetype_allowed(ultrasonic, wpilib, hal_data):
    ...
    assert ultrasonic.getPIDSourceType() == wpilib.interfaces.PIDSource.PIDSourceType.kDisplacement
...
@pytest.mark.parametrize("unit_name, expected_output", [
    ('kInches', 8.13),
    ('kMillimeters', 206.5),
    ])
