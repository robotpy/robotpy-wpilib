...
class Function:
    ...
    def from_c(cls, fn):
        ...
        returns = Type.from_c(fn['returns'],
                              True if fn['returns_pointer'] else False)
...
#         self.args = args
    ...
    def repr_py(self):
        return '%s(%s)' % (
            self.name,
            ', '.join(p.repr_py for p in self.params)
        )
    ...
    def repr_halsim(self):
        ...
        text = 'def %s(%s):' % (
            name, ', '.join(p.repr_py for p in self.params)
        )
...
def compare_fn(c_fn, py_fn, output):
    ...
    # Check return type
    output.compare("return is pointer",
                   c_fn.returns.is_ptr, py_fn.returns.is_ptr)
    
    output.compare("return type",
                   c_fn.returns.name, py_fn.returns.name)
    
    # Do the arguments match?
    c_params = c_fn.params
    py_params = py_fn.params
    
    output.compare("number of arguments",
                   len(c_params), len(py_params))
    
    for i, (c_param, py_param) in enumerate(zip(c_params, py_params)): 

        output.compare("parameter %s name" % i,
                       c_param.name, py_param.name, warning=True)
        
        output.compare("parameter %s (%s) is_ptr" % (i, c_param.name),
                       c_param.type.is_ptr, py_param.type.is_ptr)
        
        output.compare("parameter %s (%s) type" % (i, c_param.name),
                       c_param.type.name, py_param.type.name)
...
def main():
    ...
    outputs, num_errors, num_ignored_errors, num_warnings = compare(c_data, hal_data,
                                                                    False if args.filter else True)
    ...
    print()
    print("%s errors, %s ignored errors, %s warnings" % (
